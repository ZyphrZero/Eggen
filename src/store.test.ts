...
describe('reused task API profile', () => {
  ...
  it('reuses a different model on the same account without changing its saved selection', async () => {
    await reuseConfig(task({ apiProfileId: openaiProfile.id, apiModel: 'previous-model' }))
    expect(useStore.getState().reusedTaskApiProfileId).toBe(openaiProfile.id)
    expect(useStore.getState().reusedTaskApiModel).toBe('previous-model')
    expect(useStore.getState().settings.model).toBe(openaiProfile.model)
    await submitTask()
    expect(useStore.getState().tasks[0]).toMatchObject({ apiProfileId: openaiProfile.id, apiModel: 'previous-model' })
    expect(useStore.getState().settings.model).toBe(openaiProfile.model)
