...
export type SettingsTab = 'general' | 'agent' | 'api' | 'billing' | 'data' | 'about'
...
export async function initStore() {
  ...
  const storedTasks = await migrateStoredApiAccounts(useStore.getState(), await getAllTasks(), useStore.setState)
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  const { settings, prompt, inputImages, maskDraft, params, reusedTaskApiProfileId, reusedTaskApiProfileName, reusedTaskApiModel, reusedTaskApiProfileMissing, showToast, setConfirmDialog } =
    useStore.getState()
  ...
  if (normalizedSettings.reuseTaskApiProfileTemporarily && (reusedTaskApiProfileId || reusedTaskApiProfileMissing)) {
    ...
    if (!reusedProfile) {
      ...
      activeProfile = reusedTaskApiModel ? { ...reusedProfile, model: reusedTaskApiModel } : reusedProfile
...
async function executeTask(taskId: string) {
  ...
  const profile = taskProfile ?? getActiveApiProfile(settings)
  // 任务创建时已选定模型，同账号后续切换不应改变排队或恢复中的请求。
  const activeProfile = task.apiModel ? { ...profile, model: task.apiModel } : profile
...
export async function reuseConfig(task: TaskRecord) {
  ...
  const matchedProfile = taskProfile && (!isPresetConfigOnlyEnabled() || isPresetProfile(taskProfile.id))
    ? { ...taskProfile, model: task.apiModel ?? taskProfile.model }
    : null
  const shouldTemporarilyReuseProfile = Boolean(matchedProfile && (matchedProfile.id !== currentProfile.id || matchedProfile.model !== currentProfile.model))
  ...
  setReusedTaskApiProfile(
    ...
    taskProfileName,
    shouldTemporarilyReuseProfile ? matchedProfile!.model : null,
  )
  clearMaskDraft()
