...
function normalizeBalanceMapping(value: unknown): CustomProviderBalanceMapping | undefined {
  if (!isRecord(value)) return undefined
  const template = typeof value.template === 'string' ? value.template.trim() : ''
  if (!template) return undefined
  return {
    template,
    params: normalizeStringRecord(value.params),
  }
}

function normalizeProviderModels(value: unknown): CustomProviderModel[] | undefined {
  if (!Array.isArray(value)) return undefined
  const ids = new Set<string>()
  const models = value
    .map((item): CustomProviderModel | null => {
    ...
    .filter((item): item is CustomProviderModel => Boolean(item))
...
export function normalizeCustomProviderDefinition(input: unknown, usedIds = new Set<string>()): CustomProviderDefinition | null {
  ...
  return {
    ...
    balance: normalizeBalanceMapping(record.balance),
  }
}
...
export function switchApiProfileProvider(profile: ApiProfile, provider: ApiProvider, customProvider?: CustomProviderDefinition): ApiProfile {
  ...
  if (customProvider) {
    ...
    return {
      ...
      model: savedDraft?.model ?? customProvider.models?.[0]?.id ?? (shouldUseOpenAIDefaults ? DEFAULT_IMAGES_MODEL : profile.model || DEFAULT_IMAGES_MODEL),
      imageGenerationModel: savedDraft?.imageGenerationModel ?? profile.imageGenerationModel,
      apiMode: 'images',
      reasoningEffort: savedDraft?.reasoningEffort,
...
export function getCustomProviderModels(settings: Partial<AppSettings> | unknown, provider: ApiProvider): CustomProviderModel[] {
  return getCustomProviderDefinition(settings, provider)?.models ?? []
}
...
export function mergePresetImportedSettings(
  ...
  const nextProfiles = sourceProfileEntries.map((entry) => {
    ...
    if (options.lockPresetParams) return {
      ...importedProfile,
      apiKey: matched.apiKey,
      model: nextProviders.find((provider) => provider.id === importedProfile.provider)?.models?.some((model) => model.id === matched.model)
        ? matched.model
