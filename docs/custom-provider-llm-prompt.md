...
- profiles：API 账号配置数组，每个账号只保留一项，共用地址与密钥；不要为同一账号的不同模型创建多个 profile。

## customProviders 元素（Manifest）
每个元素的顶层字段：id、name、submit、editSubmit、poll、models、balance。
models 是可选模型数组，每项包含 id（请求模型 ID）、name（显示名称）、可选的 description（模型使用说明）。profile.model 是该账号当前选择的模型 ID。
balance 可选，仅在有明确账单接口且应用支持对应模板时填写；不要猜测余额查询接口。
