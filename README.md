...
同一供应商账号只需创建一个 `profile`，共用 API URL 和 API Key。可选模型写在供应商的 `models` 数组中，每项包含模型 ID `id`、显示名称 `name` 和可选的 Markdown 使用说明 `description`；`profile.model` 保存当前选择。首页按“账号 → 模型”展开切换，无需为每个模型复制连接配置。完整示例见 [豆包配置](doubao-ark-config.json)。

“账户余额”会列出所有 API 账号。支持查询的账号可单独设置账单凭证，未接入查询接口的账号会显示“暂不支持查询”。同一账号切换模型时共用余额；不同账号的余额凭证互相独立，且不会随配置导出。
