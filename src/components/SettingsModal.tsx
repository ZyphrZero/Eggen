...
import BalanceSettingsTab from './settings/BalanceSettingsTab'
...
import ModelInfoTooltip from './settings/ModelInfoTooltip'
...
const COPY_IMPORT_URL_OPTIONS_STORAGE_KEY = 'eggen.copy-import-url-options'
...
export default function SettingsModal() {
  ...
  const activeModels = getApiProfileModels(draft, activeProfile)
  const activeModelDescription = activeModels.find((model) => model.id === activeProfile.model)?.description ?? getPresetProfileDescription(activeProfile.id)
  ...
  const handleClose = () => {
    ...
    const nextDraft = {
      ...
      profiles: activeProviderIsOpenAICompatible && !activeProfileLocked
        ? getDraftWithActiveProfilePatch({ timeout: normalizedTimeout }).profiles
        : draft.profiles,
    }
  ...
  usePreventBackgroundScroll(showSettings, showZipDownloadRouteManager ? zipDownloadRouteScrollBoundaryRef : showCustomProviderImport ? customProviderScrollBoundaryRef : [settingsScrollBoundaryRef, modelInfoRef])
      ...
      >
        ...
        <div className="flex flex-1 min-h-0 flex-col sm:flex-row">
          ...
          <div className="w-full sm:w-48 shrink-0 flex flex-col border-b sm:border-b-0 sm:border-r border-gray-100 dark:border-white/[0.08] bg-gray-50/50 dark:bg-white/[0.02]">
            <nav className="flex-1 overflow-x-auto sm:overflow-y-auto custom-scrollbar p-3 space-x-1 sm:space-x-0 sm:space-y-1 flex sm:flex-col">
              ...
              <button
                ...
                className={`whitespace-nowrap flex-shrink-0 flex items-center gap-2.5 px-3 py-2.5 text-sm rounded-xl transition-colors ${activeTab === 'billing' ? 'bg-white dark:bg-white/[0.08] shadow-sm text-blue-600 dark:text-blue-400 font-medium' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100/80 dark:hover:bg-white/[0.04]'}`}
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-transparent relative overflow-hidden">
            ...
            {activeTab === 'api' && (
              ...
              <div className="block">
                <div className="mb-1.5 flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-300">
                  <label htmlFor="api-profile-model">{activeModels.length > 1 ? '当前模型' : '模型 ID'}</label>
                  {activeModelDescription && (
                    <ModelInfoTooltip key={`${activeProfile.id}:${activeProfile.model}`} content={activeModelDescription} contentRef={modelInfoRef} />
                  )}
                </div>
                {activeModels.length > 1 && (
                  <Select
                    ...
                    onChange={(value) => {
                      setReusedTaskApiProfile(null)
                      commitSettings(normalizeSettings({ ...draft, ...selectApiProfileModel(draft, activeProfile.id, String(value)) }))
                    }}
                    options={activeModels.map((model) => ({ label: model.name, value: model.id }))}
                    className="mb-2 w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200"
                  />
                )}
                <input
            ...
            {activeTab === 'billing' && <BalanceSettingsTab />}
            ...
            {activeTab === 'about' && (
              <div className="flex h-full min-h-[300px] flex-col items-center justify-center pb-8 px-6">
                ...
                */}
                ...
                <p className="mt-6 max-w-[360px] text-center text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                  本站点「蛋壳 Eggen」是基于上方开源项目修改的版本，遵循其 MIT 许可协议。
                </p>
                <p className="mt-3 mb-6 max-w-[360px] text-center text-[13px] leading-relaxed text-gray-500 dark:text-gray-400">
                  本项目的成长离不开每一位用户的使用、反馈、贡献与支持，感谢一路有你。
                </p>

                <div className="flex flex-wrap items-center justify-center gap-3">
                  <a
                    href="https://github.com/ZyphrZero/Eggen/issues"
                    target="_blank"
                    rel="noopener noreferrer"
