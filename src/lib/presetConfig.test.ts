...
describe('preset config policy', () => {
  it('allows choosing declared models on a locked account while rejecting unknown models', async () => {
    ...
    const settings = normalizeSettings({
      customProviders: [{ id: 'custom-models', name: 'Models', submit: {}, models: [{ id: 'pro', name: 'Pro' }, { id: 'lite', name: 'Lite' }] }],
      profiles: [{ id: 'account', provider: 'custom-models', model: 'lite' }],
    })
    policy.setPresetConfig(settings)
    const selected = normalizeSettings({ ...settings, profiles: [{ ...settings.profiles[0], model: 'pro' }] })
    expect(policy.enforcePresetConfigPolicy(selected).profiles[0].model).toBe('pro')
    selected.profiles[0].model = 'unknown'
    expect(policy.enforcePresetConfigPolicy(selected).profiles[0].model).toBe('lite')
  })
